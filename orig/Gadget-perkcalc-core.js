/* eslint-disable */
// This is the original core js code found on the RuneScape Wiki
// Pulled 04/15/2020
// https://runescape.wiki/w/MediaWiki:Gadget-perkcalc-core.js
// <nowiki>
;(function($, mw, rs){
	var self = {
		data: window.rsPerks,
		...
		comps: [
			{ name: "Base parts", cat: 0 },
			{ name: "Blade parts", cat: 0 },
			{ name: "Clear parts", cat: 0 },
			{ name: "Connector parts", cat: 0 },
			{ name: "Cover parts", cat: 0 },
			{ name: "Crafted parts", cat: 0 },
			{ name: "Crystal parts", cat: 0 },
			{ name: "Deflecting parts", cat: 0 },
			{ name: "Delicate parts", cat: 0 },
			{ name: "Flexible parts", cat: 0 },
			{ name: "Head parts", cat: 0 },
			{ name: "Magic parts", cat: 0 },
			{ name: "Metallic parts", cat: 0 },
			{ name: "Organic parts", cat: 0 },
			{ name: "Padded parts", cat: 0 },
			{ name: "Plated parts", cat: 0 },
			{ name: "Simple parts", cat: 0 },
			{ name: "Smooth parts", cat: 0 },
			{ name: "Spiked parts", cat: 0 },
			{ name: "Spiritual parts", cat: 0 },
			{ name: "Stave parts", cat: 0 },
			{ name: "Tensile parts", cat: 0 },
			
			{ name: "Dextrous components", cat: 1 },
			{ name: "Direct components", cat: 1 },
			{ name: "Enhancing components", cat: 1 },
			{ name: "Ethereal components", cat: 1 },
			{ name: "Evasive components", cat: 1 },
			{ name: "Healthy components", cat: 1 },
			{ name: "Heavy components", cat: 1 },
			{ name: "Imbued components", cat: 1 },
			{ name: "Light components", cat: 1 },
			{ name: "Living components", cat: 1 },
			{ name: "Pious components", cat: 1 },
			{ name: "Powerful components", cat: 1 },
			{ name: "Precious components", cat: 1 },
			{ name: "Precise components", cat: 1 },
			{ name: "Protective components", cat: 1 },
			{ name: "Refined components", cat: 1 },
			{ name: "Sharp components", cat: 1 },
			{ name: "Strong components", cat: 1 },
			{ name: "Stunning components", cat: 1 },
			{ name: "Subtle components", cat: 1 },
			{ name: "Swift components", cat: 1 },
			{ name: "Variable components", cat: 1 },
			{ name: "Third-age components", cat: 2 },
			{ name: "Armadyl components", cat: 2 },
			{ name: "Ascended components", cat: 2 },
			{ name: "Avernic components", cat: 2 },
			{ name: "Bandos components", cat: 2 },
			{ name: "Brassican components", cat: 2 },
			{ name: "Clockwork components", cat: 2 },
			{ name: "Corporeal components", cat: 2 },
			{ name: "Culinary components", cat: 2 },
			{ name: "Cywir components", cat: 2 },
			{ name: "Dragonfire components", cat: 2 },
			{ name: "Explosive components", cat: 2 },
			{ name: "Faceted components", cat: 2 },
			{ name: "Fortunate components", cat: 2 },
			{ name: "Fungal components", cat: 2 },
			{ name: "Harnessed components", cat: 2 },
			{ name: "Ilujankan components", cat: 2 },
			{ name: "Knightly components", cat: 2 },
			{ name: "Noxious components", cat: 2 },
			{ name: "Oceanic components", cat: 2 },
			{ name: "Pestiferous components", cat: 2 },
			{ name: "Resilient components", cat: 2 },
			{ name: "Rumbling components", cat: 2 },
			{ name: "Saradomin components", cat: 2 },
			{ name: "Seren components", cat: 2 },
			{ name: "Shadow components", cat: 2 },
			{ name: "Shifting components", cat: 2 },
			{ name: "Silent components", cat: 2 },
			{ name: "Undead components", cat: 2 },
			{ name: "Zamorak components", cat: 2 },
			{ name: "Zaros components", cat: 2 },
			{ name: "Classic components", cat: 2, ancient: true },
			{ name: "Historic components", cat: 2, ancient: true },
			{ name: "Timeworn components", cat: 2, ancient: true },
			{ name: "Vintage components", cat: 2, ancient: true },
		],
		...
		init: function () {
			...
			// Ancient gizmo
			var ancientGizmo = new OO.ui.CheckboxInputWidget({ selected: false, value: 1 })
			...
			for (i=0; i < self.comps.length; i++) {
				...
				var currCompEle = $('<div>', { 'class': 'perkCalcCompImg', 'data-comp': currComp.name, alt: currComp.name, draggable: true });
			...
			var showProbType = new OO.ui.CheckboxInputWidget({ selected: false, value: 1 })
			...
			self.submitBtn.$element.click(function () {
				...
				if (gizmoSelect.findSelectedItem() === null) {
					self.dispResult({ err: "Please select a gizmo" }, {});
					return
				}
			...
			permalinkBtn.$element.click(function () {
				...
				try {
					...
					mw.notify('Copied permalink to the clipboard', {tag: 'perkCalcCopyLink'});
				} catch (err) {}
				$txt.remove();
			})
			...
			var slots = $('.' + self.slotClass);
			slots.each(function () {
				$(this).append($('<div>', { 'class': 'perkCalcSlotImg' }))
			...
			try {
				...
				console.error(err)
				mw.notify('Error loading linked settings', {tag: 'perkCalcCopyLink'});
			}
        ...
        dispResult: function (result, info) {
			...
			if (result.err !== undefined) {
				...
				tbody.append(sumrow);
				var rawnoresult = 0, noresult = 0, noresult1over = '-';
				var resultsarray = [];
				for (var p in result) {
					if (result.hasOwnProperty(p)) {
						if (p === '') {
							...
							resultsarray.push( { 'perk': p, 'rawprob': result[p], 'prob': self.handlePercent(result[p]), 'oneover': self.handleOneOver(result[p]) } );
						}
					}
				}
				resultsarray.sort(function(a,b){ return b.rawprob - a.rawprob })
				...
				tbody.find('tr').click(function(){
					...
					if (tbody.find('.perkCalcSumTarget').length == 0) {
						...
						tbody.find('.perkCalcSumTarget').each(function(i,e){
							var p = parseFloat($(e).attr('data-prob')), pne = parseFloat($(e).attr('data-prob-noNE'));
							if (isNaN(p)) {
								p = 0;
							}
							if (isNaN(pne)) {
        ...
        getMaterialsProb: function (invLevel, gizmoType, matsUsed, ancient) {
        	...
        	for (var i3=0; i3 < order.length; i3++) {
				...
				for (var i5=0; i5 < zipRanks.length; i5++) {
					...
					if (probability > 0) {
						if (rank > 0) {
							probs.push({'rank': rank, 'probability': probability, 'cost': self.data.perks[perk]['ranks'][rank-1]['cost'], 'perk': perk})
						} else {
			...
			for (var i8=0; i8 < combos.length; i8++) {
				...
				self.quicksort(0, (combo.length - 1), combo, function (x, y) { return x.cost - y.cost })
				var cache_key = [];
